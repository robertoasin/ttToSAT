...
ERR:
  ...
  if (iname) {
    ...
    if (len >= 3 && !strcmp (iname + len - 3, ".gz")) {
      if (verbose >= 1) printf ("c piping '%s' through 'gzip'\n", iname);
      tmp = malloc (len + 20);
      sprintf (tmp, "gzip -c -d %s", iname);
      in = popen (tmp, "r");
      if (in) clin = 2;
      free (tmp);
    } else if (len >= 3 && !strcmp (iname + len - 3, ".xz")) {
      if (verbose >= 1) printf ("c piping '%s' through 'xz'\n", iname);
      tmp = malloc (len + 20);
      sprintf (tmp, "xz -c -d  %s", iname);
      in = popen (tmp, "r");
      if (in) clin = 2;
      free (tmp);
    } else if (len >= 4 && !strcmp (iname + len - 4, ".bz2")) {
      if (verbose >= 1) printf ("c piping '%s' through 'bzip2'\n", iname);
      tmp = malloc (len + 20);
      sprintf (tmp, "bzip2 -c -d %s", iname);
      in = popen (tmp, "r");
      if (in) clin = 2;
      free (tmp);
